...
const AddStaffForm = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append("isOnDuty", true); // default to on duty (true)
    ...
    if (response.success) {
      ...
      setErrorMsg(response.message || "Failed to add staff member.");
    }
  };

  return (
    <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50">
      <div className="bg-white rounded-xl shadow-xl w-[500px] p-6 font-sans">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto pr-1 text-left">
          <div>
          ...
          <div>
            ...
            <input
              type="text"
              name="role"
              placeholder="e.g. Chef, Admin, Helper"
              value={formData.role}
              onChange={handleChange}
              className="w-full border rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-orange-500"
              required
