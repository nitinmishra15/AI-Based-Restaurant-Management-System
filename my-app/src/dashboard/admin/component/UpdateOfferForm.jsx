...
const UpdateOfferForm = ({ offer, onClose }) => {
  ...
  useEffect(() => {
    if (offer) {
      setFormData({
        ...
        discountValue: offer.discountValue !== undefined ? offer.discountValue.toString() : "",
        minOrderAmount: offer.minOrderAmount !== undefined ? offer.minOrderAmount.toString() : "0",
        startDate: formatDate(offer.startDate),
        endDate: formatDate(offer.endDate),
        applicableCategory: offer.applicableCategory || "All",
    ...
    <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50 animate-fade-in">
      <div className="bg-white rounded-xl shadow-xl w-[520px] p-6 max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-100">
        <h2 className="text-2xl font-bold mb-5 font-sans text-gray-800 flex items-center justify-between">
          <span>Edit Promotion / Offer</span>
